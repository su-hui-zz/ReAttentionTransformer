...
def creat_model(cfg, args):
    ...
    if cfg.MODEL.POSWEIGHTS != "":
        weights = torch.load(cfg.MODEL.POSWEIGHTS, map_location='cpu')['state_dict']
        check_dict = {}
    ...
    optimizer = create_optimizer(args, model.camfuse_block)
...
def main():
    ...
    for epoch in range(1, cfg.SOLVER.NUM_EPOCHS+1):
        ...
        model.eval()
        model.module.camfuse_block.train()
...
def train_one_epoch(train_loader, model, device, criterion, optimizer, epoch,
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        cls_logits = model(input, labels=target, return_cam=False)
